...
async function fetchLobby() {
  const res = await fetch('/public/api/room_state.php?room_id=0');
  if (!res.ok) return;
  ...
  roomsEl.querySelectorAll('button[data-room]').forEach(btn => {
    btn.onclick = async () => {
      ...
      const joinRes = await fetch('/public/api/join_room.php', { method: 'POST', body: form });
      const joinData = await joinRes.json();
      if (joinData.ok) location.href = `/public/room.php?id=${btn.dataset.room}`;
...
createRoomForm?.addEventListener('submit', async (e) => {
  ...
  const res = await fetch('/public/api/create_room.php', { method: 'POST', body: form });
  const data = await res.json();
  createMsg.textContent = data.ok ? 'اتاق ساخته شد.' : data.message;
  if (data.ok) location.href = `/public/room.php?id=${data.room_id}&code=${data.invite_code}`;
});

logoutBtn?.addEventListener('click', async (e) => {
  e.preventDefault();
  await fetch('/public/api/logout.php', { method: 'POST' });
